  ...
  <div class="card border-0 shadow-sm panel-card guide-panel">
    <div class="card-body panel-body">
      ...
      <div v-show="!collapsed" class="panel-content">
        <div
          v-if="symbolEditActive"
          class="text-muted panel-subtitle d-flex align-items-center justify-content-between gap-2"
        >
          <span>Editando simbolo: {{ symbolEditName }}</span>
          <button class="btn btn-sm btn-outline-secondary" type="button" @click="exitSymbolEdit">
            Salir
          </button>
        </div>
              ...
              <div v-show="groupOpen(group.id)" class="object-children">
                ...
                >
                  ...
                  <div class="object-actions">
                    ...
                    <button
                      v-if="node.kind === 'symbol' && !symbolEditActive"
                      class="btn btn-sm btn-outline-secondary icon-btn"
                      type="button"
                      title="Editar simbolo"
                      @click.stop="editSymbol(node)"
                    >
...
onMounted(() => {
  try {
    const saved = localStorage.getItem('builder_panel_objects_collapsed')
...
watch(collapsed, (value) => {
  try {
    localStorage.setItem('builder_panel_objects_collapsed', String(value))
  } catch {
    // ignore
  }
})
...
function nodeLabel(node) {
  ...
  if (node?.kind === 'image') return node?.meta?.background ? 'Fondo' : 'Imagen'
